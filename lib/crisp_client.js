...
constructor() {
  ...
  try {
    // Korrigierte Authentifizierungsmethode laut Crisp-API-Dokumentation
    this.client.authenticateTier("plugin", {
      identifier: auth.env.CRISP_API_IDENTIFIER,
      key: auth.env.CRISP_API_KEY,
      tiers: ["websites"], // Wichtiger Parameter laut Dokumentation
      version: "1"
    });
    ...
    // Sofort einen Test-API-Aufruf durchführen, um die Authentifizierung zu verifizieren
  ...
  this.api = axios.create({
    ...
    headers: {
      "X-Crisp-Tier": "plugin",
      "X-Crisp-API-Identifier": auth.env.CRISP_API_IDENTIFIER,
      "X-Crisp-API-Key": auth.env.CRISP_API_KEY,
  ...
  this.api.interceptors.request.use(config => {
    if (this.DEBUG_MODE) {
      ...
      const maskedKey = auth.env.CRISP_API_KEY 
        ...
        : "undefined";
      logDebug(`🔑 Authentifizierungs-Headers: X-Crisp-API-Identifier=${maskedIdentifier}, X-Crisp-API-Key=${maskedKey}`);
    }
    return config;
  });
    ...
    try {
      ...
      if (error.response && error.response.status === 401) {
        ...
        try {
          ...
          // Korrigierte Authentifizierungsmethode
          this.client.authenticateTier("plugin", {
            identifier: auth.env.CRISP_API_IDENTIFIER,
            key: auth.env.CRISP_API_KEY,
            tiers: ["websites"],
  ...
  async testAuthentication() {
    try {
      // Versuche einen einfachen API-Aufruf, der Authentifizierung erfordert
  ...
  // API-Authentifizierung mit Werten aus dem auth-Modul (AKTUALISIERT)
  try {
    // Aktualisierte Authentifizierungsmethode für Crisp-API v9.10.0
    this.client.authenticate(process.env.CRISP_API_IDENTIFIER, process.env.CRISP_API_KEY);
    
    logDebug("🔐 Crisp-API-Client-Authentifizierung erfolgreich mit Methode 'authenticate'");
    
    // Sofort einen Test-API-Aufruf durchführen, um die Authentifizierung zu verifizieren
    this.testAuthentication()
      .then(success => {
        if (success) {
          logDebug("✅ Authentifizierungstest nach authenticate() erfolgreich");
        } else {
          logDebug("⚠️ Authentifizierungstest nach authenticate() fehlgeschlagen");
        }
      });
  } catch (authError) {
    const errorMsg = `❌ FEHLER bei Crisp-API-Authentifizierung: ${authError.message}`;
