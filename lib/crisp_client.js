...
constructor() {
  ...
  try {
    ...
    // Lege Website-Tier fest für die API-Berechtigungen
    ...
    // Fallback-Versuch mit alternativer Methode falls die erste fehlschlägt
    try {
      console.log("Versuche Fallback-Authentifizierung mit authenticateTier...");
      ...
      logDebug("🔐 Crisp-API-Client-Authentifizierung erfolgreich mit Fallback-Methode 'authenticateTier'");
    } catch (fallbackError) {
      const fallbackErrorMsg = `❌ Auch Fallback-Authentifizierung fehlgeschlagen: ${fallbackError.message}`;
      console.error(fallbackErrorMsg);
      logDebug(fallbackErrorMsg);
      throw fallbackError;
    }
  }
  ...
  async testAuthentication() {
    try {
      ...
      if (response.status === 200) {
        logDebug("✅ Authentifizierungstest erfolgreich");
        return true;
      } else {
        logDebug(`⚠️ Unerwarteter Status bei Authentifizierungstest: ${response.status}`);
        return false;
      }
    } catch (error) {
      logDebug(`❌ Authentifizierungstest fehlgeschlagen: ${error.message}`);
      if (error.response) {
        logDebug(`Status: ${error.response.status}, Daten: ${JSON.stringify(error.response.data || {})}`);
      }
      return false;
    }
  }
