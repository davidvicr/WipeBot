...
const auth = require("../auth"); // Importiere das auth-Modul für Zugriff auf die validierten Umgebungsvariablen
...
constructor() {
  ...
  // KRITISCHE ÄNDERUNG: Überprüfe, ob die API-Credentials vorhanden sind
  ...
  // API-Authentifizierung mit Werten aus dem auth-Modul (KORRIGIERT)
  try {
    // Aktualisierte Authentifizierungsmethode für Crisp-API
    // Verwende die neueste Methode gemäß Crisp-Dokumentation
    this.client.authenticateTier("plugin", auth.env.CRISP_API_IDENTIFIER, auth.env.CRISP_API_KEY);
    
    logDebug("🔐 Crisp-API-Client-Authentifizierung erfolgreich mit Methode 'authenticateTier'");
    
    // Sofort einen Test-API-Aufruf durchführen
    this.testAuthentication()
      .then(success => {
        if (success) {
          logDebug("✅ Authentifizierungstest nach authenticateTier() erfolgreich");
        } else {
          logDebug("⚠️ Authentifizierungstest nach authenticateTier() fehlgeschlagen");
          
          // Fallback zur älteren Authentifizierungsmethode
    ...
    // Versuche Fallback zur älteren Authentifizierungsmethode
    this.tryLegacyAuthentication();
  }
  ...
  // Verwende die korrekten Header gemäß Crisp-Dokumentation
  this.api = axios.create({
    baseURL: "https://api.crisp.chat/v1/",
    headers: {
      "X-Crisp-Tier": "plugin",
      "X-Crisp-Identifier": auth.env.CRISP_API_IDENTIFIER,
      "X-Crisp-Key": auth.env.CRISP_API_KEY,
  ...
  this.api.interceptors.request.use(config => {
    if (this.DEBUG_MODE) {
      ...
      // Zusätzliches Debug-Logging für Headers (gekürzt für Sicherheit)
      const maskedIdentifier = auth.env.CRISP_API_IDENTIFIER 
        ? `${auth.env.CRISP_API_IDENTIFIER.substring(0, 5)}...` 
        : "undefined";
      const maskedKey = auth.env.CRISP_API_KEY 
        ? `${auth.env.CRISP_API_KEY.substring(0, 5)}...` 
        : "undefined";
      logDebug(`🔑 Authentifizierungs-Headers: X-Crisp-Tier=plugin, X-Crisp-Identifier=${maskedIdentifier}, X-Crisp-Key=${maskedKey}`);
    }
    return config;
  });
...
/**
 * Versucht die ältere Authentifizierungsmethode als Fallback
 * @private
 */
tryLegacyAuthentication() {
  try {
    logDebug("🔄 Versuche Fallback zur älteren Authentifizierungsmethode...");
    ...
    // Aktualisiere auch die Axios-Instance-Header mit Basic Auth
    const authString = `${auth.env.CRISP_API_IDENTIFIER}:${auth.env.CRISP_API_KEY}`;
    const base64Auth = Buffer.from(authString).toString('base64');
    
    this.api.defaults.headers["Authorization"] = `Basic ${base64Auth}`;
    delete this.api.defaults.headers["X-Crisp-Identifier"];
    delete this.api.defaults.headers["X-Crisp-Key"];
    
    logDebug("🔐 Fallback-Authentifizierung durchgeführt");
    
    // Teste die Fallback-Authentifizierung
    this.testAuthentication()
      .then(success => {
        if (success) {
          logDebug("✅ Fallback-Authentifizierungstest erfolgreich");
        } else {
          logDebug("❌ Auch Fallback-Authentifizierung fehlgeschlagen");
        }
      });
  } catch (error) {
    logDebug(`❌ Fehler bei Fallback-Authentifizierung: ${error.message}`);
  }
}
...
async handleRateLimiting(apiCallFn, retryCount = 0) {
  try {
    ...
    if (error.response && error.response.status === 401) {
      // Maximale Anzahl von Retries begrenzen
      if (retryCount >= 3) {
        logDebug(`❌ Maximale Anzahl von Authentifizierungsversuchen erreicht (${retryCount})`);
        throw new Error(`Maximale Anzahl von Authentifizierungsversuchen erreicht: ${error.message}`);
      }
      
      logDebug(`🔐 Authentifizierungsfehler 401: ${JSON.stringify(error.response.data || {})}`);
      
      // Versuchen wir, die Authentifizierung zu erneuern
      try {
        // Direkte Authentifizierung mit der Crisp-API
        this.client = new Crisp(); // Neuen Client erstellen
        ...
        // Aktualisiere auch die Axios-Instance-Header
        this.api.defaults.headers["X-Crisp-API-Identifier"] = auth.env.CRISP_API_IDENTIFIER;
        this.api.defaults.headers["X-Crisp-API-Key"] = auth.env.CRISP_API_KEY;
        
        logDebug("🔄 Authentifizierung erneuert, versuche API-Aufruf erneut...");
        
        // Warte kurz und versuche es noch einmal
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        return this.handleRateLimiting(apiCallFn, retryCount + 1);
      } catch (authError) {
        logDebug(`❌ Fehler bei Erneuerung der Authentifizierung: ${authError.message}`);
        throw authError;
      }
    }
...
/**
 * Prüft die aktuelle Authentifizierung durch einen API-Aufruf
 * @returns {Promise<boolean>} - Erfolgsstatus der Authentifizierung
 */
/**
 * Testet die API-Authentifizierung mit einem einfachen Aufruf
 * @returns {Promise<boolean>} - true, wenn die Authentifizierung funktioniert
 */
async testAuthentication() {
  try {
    // Verwende einen einfachen API-Aufruf, um die Authentifizierung zu testen
    // Wir verwenden direkt Axios, da es mehr Kontrolle bietet
    const response = await this.api.get('user/account/check');
    
    if (response.status === 200) {
      logDebug("✅ API-Authentifizierungstest erfolgreich");
      return true;
    } else {
      logDebug(`⚠️ API-Authentifizierungstest fehlgeschlagen: Status ${response.status}`);
      return false;
    }
  } catch (error) {
    logDebug(`❌ API-Authentifizierungstest fehlgeschlagen: ${error.message}`);
    if (error.response) {
      logDebug(`📝 API-Fehler-Details: ${JSON.stringify(error.response.data || {})}`);
    }
    return false;
...
var authHeader = Buffer.from(this.identifier + ':' + this.key).toString('base64');
this.headers = {
