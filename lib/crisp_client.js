...
class CrispClient {
  constructor() {
    ...
    try {
      ...
      logDebug("🔐 Crisp-API-Client-Authentifizierung mit authenticate() durchgeführt");
      ...
      logDebug("✅ Axios-Instance mit Basic Auth konfiguriert");
    ...
    this.testAuthentication()
      .then(success => {
        if (success) {
          logDebug("✅ Initiale Authentifizierungsprüfung erfolgreich");
        } else {
          logDebug("⚠️ Initiale Authentifizierungsprüfung fehlgeschlagen");
        }
      })
      .catch(error => {
        logDebug(`❌ Fehler bei initialer Authentifizierungsprüfung: ${error.message}`);
      });
  }
