...
app.post("/hook/message", async (req, res) => {
  try {
    ...
    // Signatur-Prüfung für Webhooks (wenn Signing Secret konfiguriert ist)
    if (auth.env.CRISP_SIGNING_SECRET && !DEBUG_MODE) {
      ...
      logDebug(`🐞 Debug-Modus: Signatur-Prüfung übersprungen`);
    }
    ...
    if (origin !== "user") {
      logDebug(`⏭️ Ignoriert - Ursprung ist nicht Benutzer: ${origin}`);
